...
describe("mock-payment.service", () => {
  ...
  test("getSupportedPaymentComponents", async () => {
    ...
    expect(result?.components[3]?.type).toStrictEqual("sepa");
    expect(result?.components[4]?.type).toStrictEqual("creditcard");
    expect(result?.components[5]?.type).toStrictEqual("ideal");
    expect(result?.components[6]?.type).toStrictEqual("paypal");
